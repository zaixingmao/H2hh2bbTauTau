...
def main():
    ...
    #varList = ['svMass', 'ptJJ', 'svEta', 'iso1', 'iso2', 'J1Phi']
    varList = ['svMass', 'fMass', 'dRTauTau', 'dRJJ', 'svPt', 'dRhh']
...
#     tool.addFiles(ch=bkg2Chain, dirName="/hdfs/store/user/zmao/ZZ_3-SUB-TT", knownEventNumber=0, maxFileNumber=-1)
    ...
    iFileSig = TFile.Open("/scratch/zmao/test/H2hh300.root")
    iFileBkg1 = TFile.Open("/scratch/zmao/test/tt.root")
    iFileBkg2 = TFile.Open("/scratch/zmao/test/ZZ.root")
    ...
    ttWeight = 26.2/bkg1Chain.GetEntries() #xs (pb)
    ZZWeight = 2.5/bkg2Chain.GetEntries()  #xs (pb)
    # ====== register trees ====================================================
    #
    ...
    mycutSig = TCut( "fMass >= 50.0 && iso1<=1.5 && iso2<=1.5" ) 
    mycutBkg = TCut( "fMass >= 50.0 && iso1<=1.5 && iso2<=1.5" ) 

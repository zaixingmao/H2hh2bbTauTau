...
legendPosition = (0.5, 0.73, 0.98, 0.88)
l1 = r.TLegend(legendPosition[0], legendPosition[1], legendPosition[2], legendPosition[3])
l1.SetFillStyle(0)
l1.SetBorderSize(0)
l1.AddEntry(CutFlow_sig, "H -> hh -> #tau^{+}#tau^{-} b#bar{b}")
l1.AddEntry(CutFlow_tt, "t#bar{t} -> b#bar{b} ll")
l1.AddEntry(CutFlow_zz, "ZZ + jets -> 2q 2l")
...
c.cd(2)
CutFlow_tt.GetXaxis().SetRange(0,15)
r.gPad.SetTickx()
r.gPad.SetTicky()
r.gPad.SetLogy()
CutFlow_sig.GetXaxis().SetRange(0,15)
CutFlow_tt.SetMinimum(1)
CutFlow_tt.SetMaximum(3e6)
CutFlow_tt.Draw()

...
H_pt_h1 = r.TH1F("H_pt_h1"," ", 40, 0, 200)
H_pt_h2 = r.TH1F("H_pt_h2"," ", 40, 0, 200)
H_pt_h3 = r.TH1F("H_pt_h3"," ", 40, 0, 200)
H_pt_tt = r.TH1F("H_pt_tt"," ", 40, 0, 200)
H_pt_zz = r.TH1F("H_pt_zz"," ", 40, 0, 200)

#for normalization
HistNameList = [H_pt_h1, H_pt_h2, H_pt_h3, H_pt_tt, H_pt_zz]
xsList = (15.9, 26197.5, 2502.)

ChainHistList = [(H1Chain, signal1Location, signalEntries, H_pt_h1),
                 (H2Chain, signal2Location, signalEntries, H_pt_h2),
                 (H3Chain, signal3Location, signalEntries, H_pt_h3),
                 (ttChain, ttLocation, ttEntries, H_pt_tt),
                 (ZZChain, ZZLocation, ZZEntries, H_pt_zz)]
        ...
        if DrawSVMass:
            for iMass in range(iChain.svMass.size()):
                ...
                if diTauCandMass < diTauCandMassCut[0] or diTauCandMass > diTauCandMassCut[1]:
                    continue
                iHist.Fill((combinedTauTau + combinedJJ).pt())
        elif useVarHMass:
            ...
            for iTauPair in range(iChain.pt1.size()):
                ...
                iHist.Fill((tau1+tau2+combinedJJ).pt())
    print ''

if xsNorm:
    tool.xsNormHists(HistNameList, xsList)
    title += "; pt; Events/bin/20 fb^{-1}"
else:
    intList = tool.unitNormHists(HistNameList)
    title += "; pt; Unit Normalized"


legendPosition = (0.7, 0.75, 0.9, 0.85)#(0.45, 0.75, 0.88, 0.85)
legendHistos = [(H_pt_h1,"H260 -> hh -> #tau^{+}#tau^{-} b#bar{b}"), #(%.2f events/20 fb^{-1})" %(intList[0]*xsList[0]*20/sigPreSelection)),
                (H_pt_h2,"H300 -> hh -> #tau^{+}#tau^{-} b#bar{b}"), 
                (H_pt_h3,"H350 -> hh -> #tau^{+}#tau^{-} b#bar{b}"), 
                (H_pt_tt,"t#bar{t} -> b#bar{b} ll"),# (%.1f events/20 fb^{-1})" %(intList[1]*xsList[1]*20/ttPreSelection)),
                (H_pt_zz,"ZZ + jets -> 2q 2l")]# (%.1f events/20 fb^{-1})" %(intList[2]*xsList[2]*20/ZZPreSelection))]

H_pt_zz.SetTitle(title)
H_pt_h1.SetTitle(title)
H_pt_h1.GetYaxis().SetTitleOffset(1.2)
H_pt_h2.SetTitle(title)
H_pt_h2.GetYaxis().SetTitleOffset(1.2)
H_pt_h3.SetTitle(title)
H_pt_h3.GetYaxis().SetTitleOffset(1.2)
H_pt_zz.GetYaxis().SetTitleOffset(1.3)
...
tool.setDrawHists2(sigHist1=H_pt_h1, sigHist2=H_pt_h2, sigHist3=H_pt_h3, ttHist=H_pt_tt, ZZHist=H_pt_zz)
